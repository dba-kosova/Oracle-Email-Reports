select a.segment1 "Item",a.description "Description"
, supply_type.meaning "Supply Type"
...
, box.category_concat_segs "9 box"
...
, decode(c.on_hand_qty,0,0,round(12           /(c.on_hand_qty/decode(a.attribute7,0,1,a.attribute7)),1)) "Turns"
, decode(c.on_hand_qty,0,0,round(d.item_cost  * c.on_hand_qty,2)) "Value"
, nvl(nvl(
	(
		select line
		from
...
, case when planning_make_buy_code = 2 then (select max( sourcing_rule_name) from MRP_SR_ASSIGNMENTS_V  mis
where mis.organization_id = 85
and assignment_set_id = 1
and inventory_item_id = a.inventory_item_id) else null end "Supplier"
...
, mtl_item_categories_v box
, fnd_lookup_values_vl supply_type
where a.organization_id  = c.organization_id
    ...
    and a.organization_id   = box.organization_id(+)
    and cat.category_set_id(+) = 1100000101
    and box.category_set_id(+) = 1100000181
	and a.inventory_item_id = cat.inventory_item_id(+)
    and a.inventory_item_id = box.inventory_item_id(+)
	--and c.subinventory_code = 'DW COMP'
	and c.organization_id   = b.organization_id(+)
	and c.locator_id        = b.inventory_location_id(+) 
    and supply_type.lookup_type= 'WIP_SUPPLY'
    and supply_type.lookup_code = a.wip_supply_type
