...
, nvl(v_trx_qty,0) "Transaction Qty (3yrs)"
...
, decode(nvl(v_trx_qty,0),0,0,round (abs (trx.v_trx_qty)           / round (months_between (apps.xxbim_get_calendar_date('BIM',sysdate,-1), add_months(apps.xxbim_get_calendar_date('BIM',sysdate,-1),-36)), 1), 2)) "Act Usage"
...
,round(
		case
			when round (abs (trx.v_trx_qty) / round (months_between (apps.xxbim_get_calendar_date('BIM',sysdate,-1), add_months(apps.xxbim_get_calendar_date('BIM',sysdate,-1),-36)), 1), 2) != 0
			and on_hand_qty      > 0
			then on_hand_qty                / round (abs (trx.v_trx_qty) / round (months_between (apps.xxbim_get_calendar_date('BIM',sysdate,-1), add_months(apps.xxbim_get_calendar_date('BIM',sysdate,-1),-36)), 1), 2)
...
, (
		...
		, round (months_between (apps.xxbim_get_calendar_date('BIM',sysdate,-1), add_months(apps.xxbim_get_calendar_date('BIM',sysdate,-1),-36)), 1) v_months
		, inventory_item_id
		, organization_id
		from mtl_material_transactions mmt
		where mmt.transaction_type_id                                                                  in (35, 33) -- SO issue and WIP issue
			and trunc (mmt.transaction_date) between add_months(apps.xxbim_get_calendar_date('BIM',sysdate,-1),-36) and apps.xxbim_get_calendar_date('BIM',sysdate,-1)
