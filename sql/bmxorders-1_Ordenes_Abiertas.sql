select distinct decode(md.organization_id, 90, 'BMX',85, 'BIM')"Org", sup.order_number "Trabajo"
, (
		select order_number
			|| '-'
			|| line_number
...
, (
	...
	"Parte"
, substr(msc.description,0,20) "Descripcion"
, sup.shortage "DFF"
, to_number(nvl(old_demand_quantity,0)) "Ordene CTD"
, to_number(nvl(sup.quantity_remaining,0)) "Abierto CTD"
, sup.scheduled_completion_date "Fecha de terminado"
, sup.date_released  "Liberado"
, sup.date_completed  "Terminado"
, (
	...
	"Creado"
, request_date "Solicitud"
, schedule_ship_date "Fecha de envio"
, promise_date "Promesa"
...
where cat.organization_id = md.organization_Id
	...
	and cat.inventory_item_id = msc.sr_inventory_item_id), 'Special') "Categoria"
		, (
	...
	"Sostener"
		,decode(
						...
						, '1', 'No Ship Set', null ,'No Ship Set', 'Ship Set') "Coleccion de barcos"
...
, round(nvl(
	...
	,0),2) "Precio Unitario"
, decode (e.country, 'US', 'DOM','CA', 'INT', 'MX','INT', null, 'DOM', 'INT') "Nac/Int"
, e.party_name "Cliente"
, e2.party_name "Distribuidor"
from msc_demands md
...
, (
	...
	sup
where (md.organization_id  =  90 or (select distinct sourcing_rule_name from MRP_SR_ASSIGNMENTS_V  mis
	, msc_system_items_v msi
where mis.organization_id = md.organization_id
and msi.organization_id = mis.organization_Id
and msi.sr_inventory_item_id = mis.inventory_item_id
...
and schedule_ship_date < sysdate + 300
and request_date < sysdate + 300
--and md.inventory_item_id= '791190'
	--and e.party_name       <> 'BIM-Internal Customer'
order by schedule_ship_date asc
