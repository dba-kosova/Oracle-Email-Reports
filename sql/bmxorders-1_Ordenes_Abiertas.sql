select distinct
    ...
    (
        ...
        where
            ...
            and b.organization_id = ola.ship_from_org_id
            and assembly_item_id = ola.inventory_item_id
    ) "Line",
    msi.segment1              "Item",
    ...
    nvl((
        ...
        from
            (
                select
                    hist_creation_date, promise_date, header_id, line_id
                from
                    oe_order_lines_history h
                where
                    1 = 1
    ...
    ), promise_date) "1st Promise Date",
    nvl(decode(wdj.status_type_disp, 'Released', 'Released', 'Unreleased', 'Unreleased', 'On Hold', 'On Hold', null),(oe_line_status_pub
    .get_line_status(ola.line_id, ola.flow_status_code))) "Status",
    nvl((
        select
        ...
        where
            cat.organization_id = msi.organization_id
            and cat.structure_id = '50415'
            and cat.inventory_item_id = ola.inventory_item_id
    ), 'Special') "Category",
    ...
    (
        ...
        where
            mis.organization_id = ola.ship_from_org_id
    ...
    (
        ...
        from
            oe_lookups
        where
            ola.shipment_priority_code = lookup_code
    ...
    (
        ...
        where
            wcsm.ship_method_code = ola.shipping_method_code
            and wcsm.organization_id = ola.ship_from_org_id
    ) "Ship Method",
    round(nvl((
        select
            ola2.unit_selling_price
        from
            oe_order_lines_all ola2, oe_order_headers_all oha2
        where
            ola.header_id = oha.header_id
    ...
    (
        select
            regexp_replace(
                listagg(ohd.name, ',') within group(
                    order by
                        ohd.name asc
                ), '([^,]+)(,\1)*(,|$)', '\1\3') holds
        from
            oe_order_holds_all oeha,
            oe_hold_definitions ohd,
            oe_hold_sources_all ohsa
        where
            header_id = ola.header_id
    ...
    (
        select
            regexp_replace(
                listagg(ohd.type_code, ',') within group(
                    order by
                        ohd.type_code asc
                ), '([^,]+)(,\1)*(,|$)', '\1\3') holds
        from
    ...
    decode(nvl((
        ...
        where
            ...
            and line_code not in(
                'JIT', 'CSD', 'NJIT', 'OSV', 'FC', 'ACC'
            )
            and line_code not like 'SUB%'
            and not exists( -- this will list any "open" job (released, unreleased, on hold.. and canceled incase there was a recut)
                select
                    project_id, status_type_disp
                from
                    wip_discrete_jobs_v
                where
                    line_code = 'JIT'
        ...
        group by
            we.wip_entity_name
    ), 'No'), 'No', 'No', 'Yes') "In Staging - 90% accurate"
from
