select count(case when nvl(linepick,'asdf') = 'linepick' then 1 else null end) "Shipped from Assembly"
, count(case when  nvl(linepick,'asdf') <> 'linepick' then 1 else null end) "Went to Stock"

,round(count(case when  nvl(linepick,'asdf') = 'linepick' then 1 else null end)/ count(1) * 100,0) || '%' "% Shipped from Assy"
from (
